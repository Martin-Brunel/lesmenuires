...
# Restauration
cat backup.sql | docker compose -f infra/docker-compose.prod.yml exec -T postgres \
  psql -U "$POSTGRES_USER" "$POSTGRES_DB"
```

Volumes persistants : `pgdata` (base), `media` (photos uploadées),
`caddy_data` (certificats). **Inclure ces volumes dans la stratégie de
sauvegarde**, en plus des dumps SQL.
...
- Planifier les sauvegardes (dump SQL + volumes) et leur restauration testée.
